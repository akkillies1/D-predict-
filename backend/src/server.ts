...
app.get("/api/market/:symbol/live", async (req, res) => {
  ...
  try {
    ...
    const quote = normalizeQuote({ symbol, timestamp: marketTime, collectedAt: new Date().toISOString(), close, previousClose: Number.isFinite(previousClose) ? previousClose : null, volume: meta.regularMarketVolume == null ? null : Number(meta.regularMarketVolume), source: "yahoo-live", status: classifyFreshness(marketTime) });
