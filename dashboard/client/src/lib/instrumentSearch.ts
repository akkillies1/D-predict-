...
export async function searchInstruments(query: string, signal?: AbortSignal): Promise<Instrument[]> {
  const response = await fetch(`${API_BASE}/api/instruments/discover?q=${encodeURIComponent(query)}`, { signal });
  if (!response.ok) throw new Error(`Instrument discovery returned ${response.status}`);
