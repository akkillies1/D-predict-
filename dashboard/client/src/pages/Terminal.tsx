...
import { Activity, BarChart3, RefreshCw, Wifi, WifiOff, Zap } from "lucide-react";
import { getLatestSignal, getLocalHealth, getMarketHistory, getMarketOverview, getOptionChain, type MarketOverview, type OptionRow, type PriceBar, type Signal } from "@/lib/localApi";
...
export default function Terminal() {
  ...
  return <div className="min-h-screen bg-[#07100f] text-[#eaf4e9]">
    <header className="sticky top-0 z-30 border-b border-[#173029] bg-[#07100f]/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-3 px-4 py-3 lg:px-8">
        <div className="flex items-center gap-2 mr-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#c8f169] text-[#11210b]"><Activity size={18}/></span><b className="font-display">d—predict</b></div>
        <LiveTickerSearch value={symbol} onChange={(next) => { setSymbol(next); }} />
        <div className="ml-auto flex items-center gap-2 font-mono-ui text-[10px]">
          <span className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 ${connected ? "border-[#3d5b38] bg-[#142419] text-[#c8f169]" : "border-[#5a4328] bg-[#21180e] text-[#e5b55f]"}`}>{connected ? <Wifi size={12}/> : <WifiOff size={12}/>} {connected ? "LOCAL LIVE" : "OFFLINE"}</span>
          <button onClick={() => void refresh(true)} disabled={refreshing} className="rounded-lg border border-[#26453a] p-2 text-[#a8bdb2] hover:bg-[#12251f] disabled:opacity-50"><RefreshCw size={14} className={refreshing ? "animate-spin" : ""}/></button>
        </div>
      </div>
    </header>
    ...
    <main className="mx-auto max-w-[1600px] space-y-5 px-4 py-5 lg:px-8">
      <section className="grid gap-4 md:grid-cols-4">
        {[["SPOT", market?.close.toLocaleString("en-IN", { maximumFractionDigits: 2 }) ?? "—", market ? "LIVE" : "NO DATA"], ["OPEN", market?.open.toLocaleString("en-IN", { maximumFractionDigits: 2 }) ?? "—", "1m"], ["SIGNAL", signal?.direction ?? "—", signal ? `${Math.round(signal.confidence * 100)}%` : "NO SIGNAL"], ["UPDATED", lastUpdate ? new Date(lastUpdate).toLocaleTimeString("en-IN") : "—", latest ? "collector" : "waiting"]].map(([label, value, sub]) => <div key={label} className="rounded-xl border border-[#1d332f] bg-[#0b1714] p-4"><div className="font-mono-ui text-[9px] tracking-[.18em] text-[#70887d]">{label}</div><div className="mt-2 font-display text-2xl font-semibold">{value}</div><div className="mt-1 font-mono-ui text-[10px] text-[#879d93]">{sub}</div></div>)}
      </section>

      <section className="grid gap-5 xl:grid-cols-[1.5fr_.5fr]">
        <div className="rounded-xl border border-[#1d332f] bg-[#0b1714] p-5">
          <div className="mb-4 flex items-center justify-between"><div><div className="font-mono-ui text-[9px] tracking-[.18em] text-[#70887d]">MARKET / 1 MINUTE</div><h2 className="font-display text-xl font-semibold">{symbol}</h2></div><span className="font-mono-ui text-[10px] text-[#c8f169]">{history.length} bars</span></div>
          <div className="h-64 overflow-hidden rounded-lg border border-[#173029] bg-[#08110f] p-3"><div className="flex h-full items-end gap-1">{history.map((bar, index) => { const values = history.map((item) => item.close); const min = Math.min(...values); const max = Math.max(...values); const height = max === min ? 50 : ((bar.close - min) / (max - min)) * 85 + 8; return <div key={`${bar.timestamp}-${index}`} title={`${new Date(bar.timestamp).toLocaleTimeString("en-IN")} ${bar.close}`} className="flex-1 rounded-t bg-[#87ad5b] opacity-80" style={{ height: `${height}%` }} />; })}</div></div>
          <div className="mt-3 flex justify-between font-mono-ui text-[10px] text-[#71877d]"><span>{history[0] ? new Date(history[0].timestamp).toLocaleTimeString("en-IN") : "—"}</span><span>{latest ? new Date(latest.timestamp).toLocaleTimeString("en-IN") : "—"}</span></div>
        </div>
        <div className="rounded-xl border border-[#1d332f] bg-[#0b1714] p-5"><div className="flex items-center gap-2"><Zap size={15} className="text-[#c8f169]"/><span className="font-mono-ui text-[10px] tracking-[.16em] text-[#70887d]">SIGNAL</span></div>{signal ? <><div className={`mt-5 font-display text-3xl font-bold ${signal.direction === "BEARISH" ? "text-[#ff9d91]" : signal.direction === "BULLISH" ? "text-[#d7f883]" : "text-[#e5b55f]"}`}>{signal.direction}</div><div className="mt-2 text-sm text-[#9bb0a5]">Confidence {(signal.confidence * 100).toFixed(1)}%</div><div className="mt-5 space-y-2">{signal.reasonCodes.map((reason) => <div key={reason} className="rounded-lg border border-[#1d332f] px-3 py-2 text-xs text-[#b9c8bd]">{reason}</div>)}</div></> : <div className="mt-8 text-sm text-[#a68f62]">No stored signal for {symbol}.</div>}</div>
      </section>
