...
export default function Terminal() {
  ...
  const refresh = useCallback(async (full = true) => {
    ...
    try {
      ...
      const [quote, bars, latestSignal] = await Promise.allSettled([getMarketOverview(symbol), getMarketHistory(symbol), getLatestSignal(symbol)]);
      ...
      if (full && optionSymbols.has(symbol)) setOptions(await getOptionChain(symbol));
  ...
  useEffect(() => { const timer = window.setInterval(() => void refresh(false), 2000); return () => window.clearInterval(timer); }, [refresh]);
  useEffect(() => { const timer = window.setInterval(() => void refresh(true), 10000); return () => window.clearInterval(timer); }, [refresh]);
