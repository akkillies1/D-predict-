...
import { addInstrument, getLatestSignal, getLiveQuote, getLocalHealth, getMarketHistory, getMarketOverview, getOptionChain, type MarketOverview, type OptionRow, type PriceBar, type Signal } from "@/lib/localApi";
...
export default function Terminal() {
  ...
  const selectSymbol = useCallback(async (nextSymbol: string) => {
    ...
    try {
      await addInstrument(normalized);
      toast.success(`${normalized} activated for local collection`);
    } catch {
      toast.error(`Could not activate ${normalized}. Is the local API/database running?`);
    }
  }, []);
  ...
  const refresh = useCallback(async (full = true) => {
    ...
    try {
      ...
      const [live, stored, bars, latestSignal] = await Promise.allSettled([
        getLiveQuote(symbol),
  ...
  return <div className="min-h-screen bg-[#07100f] text-[#eaf4e9]">
    <header className="sticky top-0 z-30 border-b border-[#173029] bg-[#07100f]/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-3 px-4 py-3 lg:px-8">
        <div className="mr-2 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#c8f169] text-[#11210b]"><Activity size={18}/></span><b className="font-display">d—predict</b></div>
        ...
        <div className="ml-auto flex items-center gap-2 font-mono-ui text-[10px]">
          ...
          <button onClick={() => void refresh(true)} disabled={refreshing} aria-label="Refresh market" className="rounded-lg border border-[#26453a] p-2 text-[#a8bdb2] hover:bg-[#12251f] disabled:opacity-50"><RefreshCw size={14} className={refreshing ? "animate-spin" : ""}/></button>
        </div>
      </div>
    </header>
