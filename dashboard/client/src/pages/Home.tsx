...
import { addInstrument, getInstruments, getLocalHealth, getMarketHistory, getMarketOverview, type Instrument, type MarketOverview, type PriceBar } from "@/lib/localApi";
import {
  Activity,
  AlertTriangle,
  ArrowDownRight,
...
function OptionChain({ symbol }: { symbol: string }) {
  return <div id="chain" className="glass-panel scroll-mt-6 rounded-2xl border border-[#1d332f] p-5"><SectionHeading eyebrow="Derivatives / nearest expiry" title="Option chain" detail={`${symbol} · awaiting NSE snapshot`} action={<span className="font-mono-ui rounded-md border border-[#4e4226] bg-[#211d12] px-2 py-1 text-[10px] text-[#e5b55f]">NO DATA</span>} /><div className="rounded-xl border border-dashed border-[#315045] p-8 text-center"><BarChart3 size={20} className="mx-auto text-[#789087]" /><p className="mt-3 text-sm text-[#b7c7bd]">No collected option snapshot</p><p className="mt-1 text-xs text-[#789087]">NSE option chains are collected for NIFTY and BANKNIFTY during market hours.</p></div></div>;
}

function ForecastCard() {
  return <div id="forecast" className="glass-panel scroll-mt-6 rounded-2xl border border-[#1d332f] p-5"><SectionHeading eyebrow="Uncertainty / Monte Carlo" title="Probability cone" detail="Requires price history and computed volatility" action={<span className="font-mono-ui rounded-md border border-[#4e4226] bg-[#211d12] px-2 py-1 text-[10px] text-[#e5b55f]">NO DATA</span>} /><div className="flex h-[230px] items-center justify-center rounded-xl border border-dashed border-[#315045] text-center"><div><LineChart size={20} className="mx-auto text-[#789087]" /><p className="mt-3 text-sm text-[#b7c7bd]">Forecast unavailable</p><p className="mt-1 text-xs text-[#789087]">Collect historical bars before running <code>./dp forecast</code>.</p></div></div></div>;
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([getLocalHealth(controller.signal), getInstruments(controller.signal)]).then(([, available]) => {
      ...
      return Promise.allSettled([getMarketOverview(instrument, controller.signal), getMarketHistory(instrument, controller.signal)]);
    }).then((results) => {
      if (!results) return;
      const [overview, bars] = results;
      ...
      <div className="relative flex min-h-screen">
        ...
        <main className="relative min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1540px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            ...
            <div className="mt-6 grid gap-5 xl:grid-cols-[1.15fr_.85fr]"><PriceChart market={market} history={history} symbol={instrument} /><SignalCard symbol={instrument} /></div>
            <div className="mt-5 grid gap-5 xl:grid-cols-[1.1fr_.9fr]"><OptionChain symbol={instrument} /><TradeCard /></div>
            <div className="mt-5 grid gap-5 xl:grid-cols-2"><ForecastCard /><BacktestCard /></div>
