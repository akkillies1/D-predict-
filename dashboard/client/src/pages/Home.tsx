  ...
  return (
    <div id="overview" className="glass-panel fade-up-2 scroll-mt-6 rounded-2xl border border-[#1d332f] p-5">
      ...
      <div className="mb-4 flex flex-wrap items-end gap-x-5 gap-y-2">
        <div className="font-display text-3xl font-semibold tracking-tight text-[#edf5e9]">{market?.close != null ? market.close.toLocaleString("en-IN", { maximumFractionDigits: 2 }) : "—"}</div>
        <div className="flex items-center gap-1 text-sm text-[#c8f169]"><ArrowUpRight size={16} /> {market ? "LIVE" : "WAITING"} <span className="font-mono-ui text-xs">{market?.timestamp ? new Date(market.timestamp).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) : ""}</span></div>
        <div className="font-mono-ui text-[10px] text-[#71877d]">{market?.timestamp ? `source timestamp ${new Date(market.timestamp).toLocaleString("en-IN")}` : "waiting for local API"}</div>
      </div>
      ...
      <div className="relative flex min-h-screen">
        ...
        <main className="relative min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1540px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Spot price" value={market?.close != null ? market.close.toLocaleString("en-IN", { maximumFractionDigits: 2 }) : "—"} sub={market ? `${instrument} · live collector bar` : "Awaiting live collector bar"} trend={market ? "LIVE" : "NO DATA"} icon={TrendingUp} /><MetricCard label="Signal confidence" value="—" sub="Awaiting computed signal" trend="N/A" icon={Gauge} /><MetricCard label="Expected move" value="—" sub="Awaiting feature snapshot" trend="N/A" icon={Activity} /><MetricCard label="System health" value={apiState === "connected" ? "API OK" : "—"} sub="See data health for freshness" trend={apiState === "connected" ? "Connected" : "Offline"} icon={ShieldCheck} /></div>
