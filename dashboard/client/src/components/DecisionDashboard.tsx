...
import { addToWatchlist, getForecast, getInstruments, getLatestSignal, getLiveQuote, getLocalHealth, getMarketHistory, getMarketScan, getOptionChain, getResearch, type Forecast, type MarketOverview, type MarketPick, type OptionRow, type PriceBar, type ResearchResult, type Signal } from "@/lib/localApi";
...
export default function DecisionDashboard() {
  ...
  const [scanExcluded, setScanExcluded] = useState<Array<{ symbol: string; reason: string }>>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const [live, bars, latest, researchResult, forecastResult, scanResult] = await Promise.allSettled([
        getLiveQuote(symbol), getMarketHistory(symbol, "1d"), getLatestSignal(symbol), getResearch(symbol), getForecast(symbol, forecastHorizon), getMarketScan(5),
      ]);
      ...
      setMarketPicks(scanResult.status === "fulfilled" ? scanResult.value.picks : []);
      setScanExcluded(scanResult.status === "fulfilled" ? scanResult.value.excluded : []);
  ...
  return <div className="min-h-screen cockpit-shell text-[#eaf4e9]">
    ...
    <main className="relative mx-auto max-w-[1800px] space-y-5 px-4 py-5 lg:px-8">
      ...
      <section id="picks" className="rounded-2xl border border-[#29463b] bg-[#0b1714] p-5 shadow-[0_18px_50px_rgba(0,0,0,.16)]"><div className="flex flex-wrap items-end justify-between gap-3"><div><Label>Market-wide research scan</Label><h2 className="mt-1 font-display text-xl font-semibold">Pick of the day · up to five instruments</h2><p className="mt-1 text-xs text-[#789087]">Ranks fresh or last-session data, calibrated predictions, and positive expected return after assumed costs.</p></div><span className="rounded-full border border-[#29463b] px-3 py-1 font-mono-ui text-[9px] uppercase tracking-[.12em] text-[#c8f169]">{marketPicks.length ? `${marketPicks.length} candidates` : "NO QUALIFIED PICKS"}</span></div>{marketPicks.length ? <div className="mt-4 grid gap-3 lg:grid-cols-2">{marketPicks.map((pick, index) => <div key={pick.symbol} className="rounded-xl border border-[#1d332f] bg-[#09130f] p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-mono-ui text-[9px] text-[#70887d]">RANK {String(index + 1).padStart(2, "0")} · {pick.horizon}</div><div className="mt-1 font-display text-2xl font-semibold">{pick.symbol}</div><div className="text-[10px] text-[#789087]">{pick.name ?? "Instrument"} · data {new Date(pick.dataAsOf).toLocaleDateString("en-IN")} · {pick.dataStatus === "CLOSED_LAST_SESSION" ? "LAST SESSION" : "CURRENT"}</div></div><div className="text-right"><div className={`font-mono-ui text-sm font-semibold ${tone(pick.expectedReturn >= 0 ? "LONG" : "SHORT")}`}>{pct(pick.expectedReturn)}</div><div className="text-[10px] text-[#789087]">net {pct(pick.netExpectedReturn)}</div></div></div><div className="mt-3 grid grid-cols-3 gap-2 text-[10px]"><div><Label>Confidence</Label><div className="mt-1 text-[#d7e8d9]">{pct(pick.confidence)}</div></div><div><Label>20d momentum</Label><div className="mt-1 text-[#d7e8d9]">{pct(pick.momentum20d)}</div></div><div><Label>60d drawdown</Label><div className="mt-1 text-[#e5b55f]">{pct(pick.maxDrawdown60d)}</div></div></div><div className="mt-3 space-y-1 text-[10px] leading-relaxed text-[#aebeb3]">{pick.reasons.slice(0, 3).map(reason => <div key={reason}>• {reason}</div>)}</div><div className="mt-3 text-[10px] leading-relaxed text-[#c8b582]">Risk: {pick.risks[0]}</div></div>)}</div> : <div className="mt-4 rounded-xl border border-[#3c3120] bg-[#15120c] p-4 text-sm text-[#c8b582]">No instrument currently clears the scanner gates. This is an intentional abstention, not a missing prediction. {scanExcluded.length ? `${scanExcluded.length} candidates were excluded for insufficient history, stale data, uncalibrated probabilities, or costs.` : "The local API may be offline or the active universe has no evaluated predictions yet."}</div>}</section>
