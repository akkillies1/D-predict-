"""Evaluate whether an out-of-sample prediction ledger is promotion-ready.

The gate is deliberately conservative: a model must have enough realized
outcomes, beat the majority baseline, maintain useful directional accuracy,
and avoid obviously poor probability quality. This is an evaluation gate,
not a claim that the model is profitable.
"""
...
def evaluate(
    ...
    directional_accuracy = (
        float((directional_calls["prediction"] == directional_calls["realized_class"]).mean())
        if not directional_calls.empty else None
    )
...
def main() -> None:
    ...
    result = evaluate(args.ledger, args.min_examples, args.min_accuracy_lift, args.max_log_loss, args.min_directional_accuracy)
