...
function Ensure-Docker {
    ...
    Refresh-Path
    if (-not (Get-Command docker -ErrorAction SilentlyContinue)) { throw 'Docker Desktop was installed but docker is not on PATH. Restart Windows if necessary.' }
}
...
if (-not (Test-Path $repoMarker) -or -not (Test-Path $trainingMarker)) {
    ...
    Get-ChildItem -Force $staging | ForEach-Object { Move-Item $_.FullName $InstallDir -Force }
